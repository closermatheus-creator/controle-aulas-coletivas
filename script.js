...
function salvarMatriculaFab() {
    ...
    // Pega a grade original que você criou
    ...
    if (!codigoDigitado) { alert('⚠️ Por favor, digite o código do aluno!'); return; }
    if (!nome || !telefone) { alert('⚠️ Nome e Telefone são obrigatórios!'); return; }
    
    // Determina se ele vai pros Incompletos ou para os Ativos dependendo se você preencheu a grade
    const statusDefinido = (!seg && !ter && !qua && !qui && !sex && !sab) ? 'PENDENTE' : 'ATIVO';
